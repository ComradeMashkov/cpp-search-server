...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        if (document_id < 0) {
            throw invalid_argument("ID документа не должен быть отрицательным"s);
        }
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentPredicate document_predicate) const {
        ...
        sort(matched_documents.begin(), matched_documents.end(),
             [](const Document& lhs, const Document& rhs) {
                 if (abs(lhs.relevance - rhs.relevance) < MIN_COMPARISON_TOLERANCE) {
...
public:
    Paginator(Iterator begin, Iterator end, size_t page_size) {
        for (size_t i = distance(begin, end); i > 0;) {
            const size_t current_page_size = min(page_size, i);
...
public:
    ...
    vector<Document> AddFindRequest(const string& raw_query, DocumentPredicate document_predicate) {
        vector<Document> documents = search_request_.FindTopDocuments(raw_query, document_predicate);
