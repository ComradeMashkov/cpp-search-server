...
void SearchServer::RemoveDocument(int document_id) {
    if (document_ids_.find(document_id) != document_ids_.end()) {
        for (const auto [word, _] : word_to_document_freqs_ids_[document_id]) {
            const auto word_to_erase = word_to_document_freqs_[word].find(document_id);
            word_to_document_freqs_[word].erase(word_to_erase);
        }
    }

    documents_.erase(document_id);
    document_ids_.erase(document_id);
    word_to_document_freqs_ids_.erase(document_id);
}
...
std::vector<std::string> SearchServer::SplitIntoWordsNoStop(const std::string& text) const {
    ...
    for (const std::string& word : SplitIntoWords(text)) {
        if (!IsValidWord(word)) {
            throw std::invalid_argument(ShieldString("В слове \""s + word + "\" присутствуют недопустимые символы"s));
        }
        if (!IsStopWord(word)) {
...
SearchServer::QueryWord SearchServer::ParseQueryWord(std::string text) const {
    ...
    if (!IsValidWord(text)) {
        throw std::invalid_argument(ShieldString("В слове \""s + text + "\" запроса содержатся недопустимые символы"s));
    }
